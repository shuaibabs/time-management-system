...
function getJwtPayload(token) {
    ...
    try {
        ...
        //validating token
        if (token == '') {
            throw 'AUTH ERROR: BLANK: No Valid Authentication token paased in request';
        }
        if (token == 'undefined') {
            throw 'AUTH ERROR: UNDEFINED: No Valid Authentication token paased in request';
        }
        if (!token) {
            throw 'AUTH ERROR: No Valid Authentication token paased in request';
        }
